...
int main(){ // Калькулятор. Обработка деления на 0, выход по нажатию клавиши 'e' вместо операции, бесконечное вычисление до операции '='
	...
	//
	//Это типа себы похвалил? Или это похволился не ты, а тот, кто писал тебе этот ткод????
	//
	char operation = 0;
	...
	while (operation != '='){
		...
		case '/':
			if (b == 0) {
				...
				(float)result = (float)a / b; // не понял зачем приведение типов, если объявлены пернменные float
			break;
		case 'c': result = 0;
			cout << "Обнуление\n";
			cout << "Значение: ";
